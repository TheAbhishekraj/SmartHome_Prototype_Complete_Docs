# 13_Cost_Estimate_Summary.md
# SmartHome Prototype — Complete Cost Analysis

**Version:** 1.0 | **Date:** August 2026

---

## Prototype Cost (2-Floor Flat)

| Category | ₹ (INR) | $ (USD) |
|----------|---------|---------|
| ESP32/ESP8266 Nodes (15 units) | ₹4,500 | $54 |
| Sensors (all types) | ₹8,000 | $96 |
| Actuators (relays, valves, motors) | ₹6,000 | $72 |
| Power Supplies & Distribution | ₹3,000 | $36 |
| Enclosures & Mounting | ₹2,500 | $30 |
| Cables & Connectors | ₹2,000 | $24 |
| Safety-Critical Certified Parts (incl. +₹2,000 safety buffer)* | ₹7,000 | $84 |
| Common Tools & Consumables | ₹3,120 | $37 |
| Scrap Reuse (PC, routers, phones) | ₹0 | $0 |
| **Subtotal** | **₹36,120** | **$433** |
| Contingency (10%) | ₹3,612 | $43 |
| **GRAND TOTAL** | **₹39,732** | **$476** |

*\*Note: The safety-critical components budget has been padded by +₹2,000 (raising it from ₹5,000 to ₹7,000) as recommended in pre-build audits. This ensures procurement of authentic, certified protective components (e.g. ISI/BIS marked gas solenoids, heavy-duty contactors, and high-quality fuses) to guarantee system reliability when interacting with mains utilities and hazardous services.*

---

## Cost Per Zone

| Zone | Nodes | Cost (₹) |
|------|-------|----------|
| Garden/Balcony | 1 | ₹2,660 |
| Kitchen | 2 | ₹3,750 |
| Water/Pump | 1 | ₹1,760 |
| Living Room | 1 | ₹1,400 |
| Bedrooms | 2 | ₹1,710 |
| Bathrooms | 2 | ₹2,340 |
| Security | 4 | ₹5,360 |
| Air Quality | 1 | ₹4,030 |
| Aquarium | 1 | ₹1,360 |
| Safety (Whole House) | 5 | ₹5,730 |
| Common/Tools | — | ₹3,120 |

---

## Commercial Kit Cost Projection (100+ units)

| Item | Per-Unit Cost (₹) |
|------|-------------------|
| Custom PCB + components | ₹900 |
| Enclosure (injection molded) | ₹400 |
| Pre-flashed ESP32 module | ₹300 |
| Sensors (bulk) | ₹500 |
| Actuators (bulk) | ₹600 |
| Power supply (shared per home) | ₹250 |
| Packaging & docs | ₹250 |
| Assembly labor | ₹300 |
| **Total Kit Cost** | **₹3,500** |
| Hub (old PC not included) | ₹0 |
| **Per-Home System Cost** | **₹22,000** |
| **Target Selling Price** | **₹55,000** |
| **Gross Margin** | **60%** |

---

## Scaling Economics

| Volume | Kit Cost | Selling Price | Margin |
|--------|----------|---------------|--------|
| 10 units | ₹28,000 | ₹55,000 | 49% |
| 50 units | ₹24,000 | ₹55,000 | 56% |
| 100 units | ₹22,000 | ₹55,000 | 60% |
| 500 units | ₹18,000 | ₹50,000 | 64% |

---

*End of 13_Cost_Estimate_Summary.md*
